...
def interseg(realuser, nclicks):
    ...
    print("Iniciando o pré-treino do modelo base m2...")
    
    from utilIO import getTrainData
    print("Carregando dados da pasta 'train/'...")
    train_images, train_labels_one_hot, train_labels_num, _ = getTrainData(N, h1, w1, dimcls)

    print(f"Dados carregados. {len(train_images)} imagens de treino encontradas.")
    print("Iniciando o treino. Isto pode demorar vários minutos...")

    # --- START OF THE FIX ---
    # Use train_labels_num (the integer labels) instead of the one-hot version
    m2.fit(train_images, {'lnow': train_labels_num}, epochs=100, batch_size=Nbatch, verbose=1)
    m2.save_weights('auxiliary/m2.h5')
    print("Modelo m2 pré-treinado e salvo em auxiliary/m2.h5!")
    import sys
    sys.exit()
    ...
    print("--- INICIANDO O PRÉ-TREINO DO MODELO BASE (m2) ---")
    from utilIO import getTrainData
    from config import N, h1, w1, dimcls, Nbatch
    ...
    print("Carregando dados da pasta 'train/'...")
    train_images, train_labels, _, _ = getTrainData(N, h1, w1, dimcls)

    print(f"Dados carregados. {len(train_images)} imagens de treino encontradas.")
    print("Iniciando o treino. Isto pode demorar vários minutos...")

    m2.fit(train_images, {'lnow': train_labels}, epochs=100, batch_size=Nbatch, verbose=1)

    m2.save_weights('auxiliary/m2.h5')
    print("\n--- PRÉ-TREINO CONCLUÍDO! ---")
    print("Modelo 'm2.h5' salvo com sucesso na pasta 'auxiliary/'.")

    import sys
    print("Pode agora reverter as alterações no 'seg.py' e executar o programa principal.")
    sys.exit()
