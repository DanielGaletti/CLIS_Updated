...
def get_deeplab_model(num_classes=21, pretrained=False):
    weights = torchvision.models.segmentation.DeepLabV3_ResNet50_Weights.DEFAULT if pretrained else None
    model = torchvision.models.segmentation.deeplabv3_resnet50(weights=weights, progress=False, aux_loss=True)
...
def predict(model, image_tensor, device):
    ...
    return mask
# In your run_experiments_dissertation.py file

def finetune_on_clicks(model, optimizer, image_tensor, user_clicks, device, iterations=15):
    """Refina o modelo, garantindo que as coordenadas dos cliques são inteiras e dentro dos limites."""
    model.train()
    def set_bn_eval(m):
    ...
    for p in user_clicks['pos']:
        y = max(0, min(int(p[0]), max_y))
        x = max(0, min(int(p[1]), max_x))
        click_mask[y, x] = 1
...
def calculate_iou(pred_mask, gt_mask, class_id):
    if class_id is None: return np.nan
...
# --- LÓGICA DE EXPERIMENTAÇÃO COM APRENDIZAGEM ADAPTATIVA ---
def run_single_experiment(image_path, gt_mask_path, object_class_id, simulated_clicks, base_model_path=None, model_to_use=None):
    ...
    if model_to_use is None:
        model = get_deeplab_model(num_classes=NUM_CLASSES, pretrained=False).to(device)
        if base_model_path:
    ...
    if simulated_clicks['pos'] or simulated_clicks['neg']:
        # *** LÓGICA DE APRENDIZAGEM ADAPTATIVA ***
        if initial_iou is not None and not np.isnan(initial_iou) and initial_iou > 0.85:
            learning_rate = 2e-5 # Refinamento GENTIL para especialistas
            print(f"INFO: IoU inicial alto ({initial_iou:.4f}). Usando taxa de aprendizagem GENTIL: {learning_rate}")
        else:
            learning_rate = 1e-4 # Refinamento NORMAL para modelos com espaço para melhoria
            print(f"INFO: IoU inicial ({initial_iou:.4f}). Usando taxa de aprendizagem NORMAL: {learning_rate}")

        for param in model.backbone.parameters(): param.requires_grad = False
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate)
        finetune_on_clicks(model, optimizer, image_tensor, simulated_clicks, device)
    
    final_iou = calculate_iou(predict(model, image_tensor, device), gt_mask, object_class_id)
    
    return initial_iou, final_iou, model

# (O resto do script, run_dissertation_experiments e o painel de controlo, permanecem os mesmos)
def run_dissertation_experiments(experiments_config, base_model_path):
    ...
    for exp_name, config in experiments_config.items():
        ...
        if config['type'] == 'efficiency':
            ...
            gain = ((final_iou - initial_iou) / initial_iou) * 100 if initial_iou is not None and not np.isnan(initial_iou) and initial_iou > 0 else 0.0
            ...
            config['params_exp_part2']['base_model_path'] = None
    ...
    df.to_csv('results/dissertation_results_adaptive.csv', index=False)
    print("\n✅ Experimentos concluídos. Resultados salvos em 'results/dissertation_results_adaptive.csv'")
    print(df)
...
if __name__ == '__main__':
    ...
    EXPERIMENTS = {
        'Exp1_Cavalo': {
            'type': 'efficiency',
            'params': { 'image_path': 'test_data/2007_000033.jpg', 'gt_mask_path': 'test_data/2007_000033.png',
                'object_class_id': 0,
                'simulated_clicks': {'pos': [(150, 180), (110, 220)], 'neg': [(50, 50), (100, 125)]}
            }
        },
        'Exp2_Continual_Cavalo': {
            'type': 'continual',
            'params_control': { 'image_path': 'test_data/2007_000256.jpg', 'gt_mask_path': 'test_data/2007_000256.png',
                'object_class_id': 0, 'simulated_clicks': {'pos':[],'neg':[]}
            },
            'params_exp_part1': { 'image_path': 'test_data/2007_000033.jpg', 'gt_mask_path': 'test_data/2007_000033.png',
                'object_class_id': 0,
                'simulated_clicks': {'pos': [(150, 180)], 'neg': [(100, 125)]}
            },
            'params_exp_part2': { 'image_path': 'test_data/2007_000256.jpg', 'gt_mask_path': 'test_data/2007_000256.png',
                'object_class_id': 0, 'simulated_clicks': {'pos':[],'neg':[]}
            }
        },
         'Exp3_Sofa_com_dificuldade': { 
            'type': 'efficiency',
            'params': {
            'image_path': 'test_data/2007_001149.jpg',
            'gt_mask_path': 'test_data/2007_001149.png',
            'object_class_id': 0,
            'simulated_clicks': {'pos': [(279, 132)], 'neg': [(152, 103)]} 
            }
        }
    }
